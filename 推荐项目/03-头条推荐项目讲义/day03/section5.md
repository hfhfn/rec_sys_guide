...
####1、创建spark streaming配置信息、Kafka连接以及happybase
...
* from pyspark.streaming import StreamingContext(sparkContext, batchDuration=None)

  * sparkContext: SparkContext对象
  * batchDuration=None:设置了BatchDuration，Job每隔BatchDuration时间会自动触发
    * Spark Streaming需要指定处理数据的时间间隔，如果`1s`，那么Spark Streaming会以`1s`为时间窗口进行数据处理。此参数需要根据用户的需求和集群的处理能力进行适当的设置。
  * `start`()：开启Sreaming处理，`ssc.start()`启动后程序才真正进行所有预期的操作。

  - `stop`(*stopSparkContext=True*):停止 streams
...
# 创建Streaming Context
stream_c = StreamingContext(sc, 5)
```

**配置streaming 读取Kafka的配置,在配置文件中增加KAFKAIP和端口**

* KafkaUtils.createDirectStream(ssc, [topic], kafkaParams)
  * ssc:Sparkstreaming对象
  * topic:Kafka已经开启的主题名称
  * kafkaParams：kafka配置参数：IP：PORT，"group.id"分组名称
