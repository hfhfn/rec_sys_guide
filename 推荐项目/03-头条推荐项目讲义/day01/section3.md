...
我们建立推荐系统项目工程目录（会按照业务功能进行区分），以下是我们后面完整的工程目录：
...
* 关键词：关键词则是作者自由定义的，通常是文章主题内容的高度概括，比主题词更明确、更具体。
  * 通过分词+TFIDF/TextRank获取文章中一些词的权重高的。

* 主题词：主题词是由机构定义和发布的规范词，通常是专有名词或名词短语；
  * 进行规范化处理的，文章中出现的不同方法结果中同义词或者同时出现的词
...
文章画像结果存储在HIVE当中，后续作为离线特征分析的数据源。HIVE中有一个article数据库，存储文章相关中间计算结果以及画像结果；
...
* 目的：计算文章的关键词以及主题词结果
  ...
  * 主题词：TextRank的TOPK词 与 ITFDF计算的TOPK个词的交集
* 步骤：
  * 1、计算历史所有文章的TFIDF与TextRank
  * 2、通过TFIDF与TextRank计算关键词与主题词

### 2.4.1 离线计算历史所有文章的TFIDF与TextRank
...
* 离线文章数量过多，如果一个spark程序完全计算，那么程序所需内存过大，结果最好中间存储下来

![](../images/计算关键词中间结果.png)
...
* 2、TFIDF模型训练保存，spark使用count与idf进行计算
  * TFIDF计算方案：
  * **先计算分词之后的每篇文章的词频，得到CV模型**
  * **然后根据词频计算IDF以及词，得到IDF模型**
* 3、使用模型计算N篇文章数据的TFIDF值
...
# HIVE数据仓库已经有这个表以及之前计算好的所有频道的文章结果，这里做测试就不在进行插入了
...
![](../images/%E7%BD%91%E9%A1%B5pagerank.png)

* 算法原理
  * 1、PageRank算法：就是预先给每个网页一个PR值（下面用PR值指代PageRank值），由于PR值物理意义上为一个网页被访问概率，所以一般是1/N，其中N为网页总数（初始概率）。
...
* 其中Mpi是所有对pipi网页有出链的网页集合，L(pj)是网页pj的出链数目，N是网页总数，α一般取0.85。 

例子：

![](../images/PR计算图.png)

三个页面A、B、C为了便于计算，我们假设每个页面的PR初始值为1，α为0.5
...
1、首先对这句话分词，这里可以借助各种分词项目，比如HanLP、Jieba分词，得出分词结果：
...
- 步骤：
  - 1、加载IDF，保留关键词以及权重计算(TextRank * IDF)？为什么？合并关键词权重到字典结果
  - 2、将tfidf和textrank共现的词作为主题词
